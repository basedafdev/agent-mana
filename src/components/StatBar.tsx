...
function getHpColor(remaining: number): { gradient: string; label: string; pulse: string } {
  if (remaining >= 75) {
    return { gradient: 'from-green-400 to-emerald-600', label: 'text-green-400/80', pulse: 'bg-green-500/20' };
  } else if (remaining >= 50) {
    return { gradient: 'from-yellow-400 to-amber-500', label: 'text-yellow-400/80', pulse: 'bg-yellow-500/20' };
  } else if (remaining >= 25) {
    return { gradient: 'from-orange-400 to-orange-600', label: 'text-orange-400/80', pulse: 'bg-orange-500/20' };
  } else {
    return { gradient: 'from-red-400 to-red-600', label: 'text-red-400/80', pulse: 'bg-red-500/20' };
  }
}

function getMpColor(remaining: number): { gradient: string; label: string } {
  if (remaining >= 75) {
    return { gradient: 'from-blue-500 to-indigo-600', label: 'text-blue-400/80' };
  } else if (remaining >= 50) {
    return { gradient: 'from-blue-400 to-blue-500', label: 'text-blue-400/80' };
  } else if (remaining >= 25) {
    return { gradient: 'from-sky-400 to-blue-400', label: 'text-sky-400/80' };
  } else {
    return { gradient: 'from-cyan-300 to-sky-400', label: 'text-cyan-300/80' };
  }
}
...
export default function StatBar({ type, utilization, label, resetTime }: StatBarProps) {
  ...
  return (
    <div className="space-y-1 relative">
      ...
      <div 
        className="relative h-3 bg-black/40 border border-white/10 overflow-hidden cursor-pointer"
      ...
      >
        ...
        {showTooltip && resetTime && (
          <div className="absolute -top-8 left-1/2 -translate-x-1/2 px-2 py-1 
                        bg-black/90 border border-white/10 rounded text-[9px] text-white/70 
