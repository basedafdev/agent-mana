...
use super::{ProviderStatus, ClaudeUsageSnapshot, NotificationService};
use crate::tray;
...
impl PollingService {
    ...
    pub async fn start(self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                Self::poll_anthropic(&anthropic_status, &notification_service, &app_handle).await;
    ...
    async fn poll_anthropic(
        ...
        if !ClaudeOAuthClient::has_credentials() {
            Self::handle_no_credentials(anthropic_status, app_handle).await;
            return;
        }

        match ClaudeOAuthClient::from_credentials_file() {
            Ok(client) => {
                Self::fetch_and_update_usage(client, anthropic_status, notification_service, app_handle).await;
            }
            Err(e) => {
                Self::handle_oauth_error(e, anthropic_status, app_handle).await;
            }
        }
    }
    ...
    async fn fetch_and_update_usage(
        ...
        match client.get_usage().await {
            Ok(usage) => {
                ...
                Self::update_tray_connected(weekly_util.unwrap_or(0.0), period_util, app_handle);
    ...
    async fn handle_oauth_error(
        ...
        Self::update_tray_disconnected(app_handle);
    }
    ...
    fn update_tray_connected(weekly_util: f64, period_util: f64, app_handle: &AppHandle) {
        let weekly_remaining = 100.0 - weekly_util;
        let period_remaining = 100.0 - period_util;
        let icon_data = tray::generate_tray_icon(weekly_remaining, period_remaining);
    ...
    fn update_tray_disconnected(app_handle: &AppHandle) {
    ...
    async fn poll_openai(
        ...
        if let Ok(Some(api_key)) = keychain.get_api_key("openai") {
            let client = crate::api::openai::OpenAIClient::new(api_key);
